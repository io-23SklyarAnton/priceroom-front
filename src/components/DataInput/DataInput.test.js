...
describe('DataInput Component', () => {
  ...
  test('should render form fields and submit button', () => {
    ...
    expect(
      screen.getByLabelText(/Загальна кількість поверхів/),
    ).toBeInTheDocument();
    expect(screen.getByLabelText(/Кількість кімнат/)).toBeInTheDocument();
    expect(
      screen.getByLabelText(/Загальна площа квартири/),
    ).toBeInTheDocument();
  ...
  test('should show error messages for invalid input', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Поверх/), {
      target: { value: '0' },
    });
    fireEvent.change(screen.getByLabelText(/Загальна кількість поверхів/), {
      target: { value: '0' },
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Поверх має бути в межах від 1 до 100/),
      ).toBeInTheDocument();
      expect(
        screen.getByText(
  ...
  test('should submit form and display predicted price', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Кількість кімнат/), {
      target: { value: '3' },
    });
    fireEvent.change(screen.getByLabelText(/Загальна площа квартири/), {
      target: { value: '100' },
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Прогнозована ціна: 100000.00 дол./),
      ).toBeInTheDocument();
    });
  });
  ...
  test('should handle errors when API fails', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Не вдалося отримати прогноз ціни або зберегти дані./),
      ).toBeInTheDocument();
    });
  });
});
